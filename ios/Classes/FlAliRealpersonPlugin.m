#import "FlAliRealpersonPlugin.h"
  ...
  if ([@"startRealPerson" isEqualToString:call.method]) {
    ...
    [RPSDK startWithVerifyToken:call.argument[@"token"]
        ...
        switch (result.state) {
            case RPStatePass:
                // 认证通过。
                [channel invokeMethod:@"onRealPersonResult" arguments:@"SUCCESS"];
                break;
            case RPStateFail:
                // 认证不通过。
                 [channel invokeMethod:@"onRealPersonResult" arguments:@"FAIL"];
                break;
            case RPStateNotVerify:
                // 未认证。
                // 通常是用户主动退出或者姓名身份证号实名校验不匹配等原因导致。
                // 具体原因可通过result.errorCode和result.message来区分（详见错误码说明）。
                [channel invokeMethod:@"onRealPersonResult" arguments:@"CANCLE"];
                break;
        }
    }];
  } else {
